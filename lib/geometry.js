...
Projection.prototype = {
    ...
    overlapsCompletely: function (projection) {
        return (this.min >= projection.min && projection.max >= this.max) || (this.max >= projection.max && projection.min >= this.min);
    }
};
...
Polygon.prototype.init = function () {
    var options = arguments[0] || {};
    var pts = options.vertices instanceof Array && options.vertices || []; // argument should be 1D-Array, [x0, y0, x1, y1, ...]
    if (pts.length) {
        for (var i = 0; i < pts.length; i += 2) {
            this.addPoint(pts[i], pts[i + 1]);
        }
    }
...
Polygon.prototype.move = function (dTime, dVector) {
    var pts = this.vertices,
        velocity = dVector || this.velocity.scale(dTime);
    for (var i = 0; i < pts.length; i++) {
        pts[i].x += velocity.x;
        pts[i].y += velocity.y;
    }
