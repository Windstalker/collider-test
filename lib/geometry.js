...
Shape.prototype = {
    ...
    collidesWithBorders: function (parentShape) {
        var bBox = this.getBoundingBox(),
			...
			dx0 = bBox.x.min - pBox.x.min,
			dx1 = pBox.x.max - bBox.x.max,
			dy0 = bBox.y.min - pBox.y.min,
			dy1 = pBox.y.max - bBox.y.max,
            outOfX = !(dx0 >= 0 && dx1 >= 0),
            outOfY = !(dy0 >= 0 && dy1 >= 0);
        if (outOfX || outOfY) {
            var mtvX = +outOfX * Math.min(dx0, dx1);
            var mtvY = +outOfY * Math.min(dy0, dy1);
			var mtv = this.velocity.normalize().invert().scale(new Vector(mtvX, mtvY).getMagnitude());

            this.move(0, mtv);

            if (outOfX) { this.velocity = this.velocity.invertX(); }
            if (outOfY) { this.velocity = this.velocity.invertY(); }
        }
    },
    ...
    move: function (dt, dV) {
        throw 'move(dt, dV) not implemented';
    },
// Drawing methods
...
Polygon.prototype.move = function (dt, dV) {
    var v = this.vertices,
        velocity = dV || this.velocity.scale(dt);
    for (var i = 0; i < v.length; i++) {
        v[i] = v[i].add(velocity);
    }
};
...
Circle.prototype.move = function (dt, dV) {
	var velocity = dV || this.velocity.scale(dt);
